...
// M11.1 integration coverage: POST /api/v1/reviews/:id/retry enforces the
// chain-step policy gate when the review belongs to a chain_run. Mirrors
// decide-chain-policy.test.ts since both endpoints route through the same
// `assertChainStepAllows` helper.
...
describe("POST /api/v1/reviews/:id/retry — chain-step policy gate (M11.1)", () => {
  let app: any;
  let db: any;
  ...
  beforeAll(async () => {
    ...
    templateSlug = "m11_retry_chain_tpl";
    ...
    aliceEmail = "alice-m11r@example.com";
    bobEmail = "bob-m11r@example.com";
    adminEmail = "admin-m11r@example.com";
    ownerEmail = "owner-m11r@example.com";
  ...
  it("allows /retry when session email matches step assignee", async () => {
    ...
    const res = await request(app)
      ...
      .send({ feedback: "please tweak the wording" });

    expect(res.status).toBe(200);
    expect(res.body?.id || res.body?.review?.id).toBe(reviewId);
  });

  it("denies /retry when session email does not match (and not admin/owner)", async () => {
    const { reviewId } = await createChainRun(db, {
      projectId,
      createdBy: `reviewer:${ownerEmail}`,
      step1Assignee: { kind: "user", email: aliceEmail },
    ...
    const res = await request(app)
      ...
      .send({ feedback: "trying to butt in" });

    expect(res.status).toBe(403);
  });

  it("admin bypass on /retry", async () => {
    const { reviewId } = await createChainRun(db, {
      projectId,
      createdBy: `reviewer:${ownerEmail}`,
      step1Assignee: { kind: "user", email: aliceEmail },
