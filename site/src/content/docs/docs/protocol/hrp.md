---
title: "HRP: Human Review Protocol"
description: The open specification for Agent↔Human communication. MCP standardizes Agent↔Tool. A2A standardizes Agent↔Agent. HRP standardizes Agent↔Human.
---

<!-- This page mirrors docs/protocol/hrp-v1.md in the repository root. -->
<!-- The repo file is the canonical source: edit there, then update this page. -->

## Why another protocol?

The AI agent ecosystem already has protocols for two of the three communication directions:
...
The Agent ↔ Human handoff has been the missing layer. Every team building agent systems ends up re-inventing the same wiring: a way for an agent to pause, hand work to a human for a decision, and resume. HRP formalizes that pattern into a minimal, transport-agnostic spec so that the wiring is interoperable rather than bespoke.

The canonical specification lives in the Gatewerk repository: [`docs/protocol/hrp-v1.md`](https://github.com/gatewerk/gatewerk/blob/master/docs/protocol/hrp-v1.md).

Gatewerk is the reference implementation. Any system that implements this spec is HRP-compatible.
